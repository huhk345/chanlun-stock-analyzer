...
function extractEmailSummary(content: string): string {
  ...
  if (dateMatch) {
    summary.push(`ChanLun Daily Report — ${dateMatch[1]}`);
    summary.push('');
  }
  ...
  for (const section of stockSections) {
    ...
    summary.push(`[${code}] Close: ${price}  Change: ${change}`);
    summary.push('');
    
    const overviewMatch = section.match(/# 总览\s*\n([\s\S]*?)(?=\n##|$)/);
    if (overviewMatch) {
      const overview = overviewMatch[1].trim();
      const lines = overview.split('\n').filter(l => l.trim() && !l.startsWith('>')).slice(0, 5);
      for (const line of lines) {
        summary.push(`  ${line.replace(/^[-•]\s*/, '').trim()}`);
      }
      summary.push('');
    }
  }
  ...
  summary.push('━━━━━━━━━━━━━━━━━━━━━━━━');
  summary.push('Full report attached.');
  summary.push('');
  summary.push('*Auto-generated by ChanLun Analysis System. Not investment advice.*');
